	...
	while (i < side) //  переход на новую строку (строка)
	{
		...
		while (j < side) // рисует строку (столбец)
		{
			if (i == 0 || i == side - 1 || j == 0 || j == side - 1 || // пустой квадрат
				i <= side / 4 && j >= side / 4 * 3 || // правый верхний залитый 
				i <= side / 4 && j <= side / 4 || // левый верхний залитый
				i >= side / 4 * 3 && j >= side / 4 * 3 || // правый нижний залитый
				i >= side / 4 * 3 && j <= side / 4  // левый нижний залитый
				)
				cout << "* ";
			else
				cout << "  ";
